...
var add_wall = function() {
    ...
    if (bias_x > 0.0)
        block_a.scale.z = (change_time[cnt + 1] - change_time[cnt]) * 0.001 * velocity;
    else
        block_a.scale.x = (change_time[cnt + 1] - change_time[cnt]) * 0.001 * velocity;
    block_a.position.set(
        cube.position.x + cube.scale.x * 0.5 + block_a.scale.x * 0.5 + (bias_x - 0.75) * 2,
        0.5,
        cube.position.z + cube.scale.z * 0.5 + block_a.scale.z * 0.5 + (bias_z - 0.75) * 2
    );
    scene.add(block_a);
    ...
    if (bias_x > 0.0)
        block_b.scale.z = (change_time[cnt + 1] - change_time[cnt]) * 0.001 * velocity;
    else
        block_b.scale.x = (change_time[cnt + 1] - change_time[cnt]) * 0.001 * velocity;
    block_b.position.copy(block_a.position);
    if (bias_x > 0.0) {
        block_b.position.x -= 3.0;
        block_b.position.z += 3.0;
    } else {
        block_b.position.x += 2.0;
        block_b.position.z -= 3.0;
    }
    scene.add(block_b);
